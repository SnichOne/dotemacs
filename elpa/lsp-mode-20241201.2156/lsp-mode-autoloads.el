...
(defvar lsp-inline-completion-active-map (let ((map (make-sparse-keymap))) (define-key map (kbd "C-<return>") #'lsp-inline-completion-accept) (define-key map [mouse-1] #'lsp-inline-completion-accept-on-click) (define-key map (kbd "C-n") #'lsp-inline-completion-next) (define-key map (kbd "C-p") #'lsp-inline-completion-prev) (define-key map (kbd "C-g") #'lsp-inline-completion-cancel) (define-key map (kbd "<escape>") #'lsp-inline-completion-cancel) (define-key map (kbd "C-c C-k") #'lsp-inline-completion-cancel) (define-key map (kbd "C-l") #'recenter-top-bottom) (define-key map [down-mouse-1] #'ignore) (define-key map [up-mouse-1] #'ignore) (define-key map [mouse-movement] #'ignore) (define-key map [t] #'lsp-inline-completion-cancel-with-input) map) "\
Keymap active when showing inline code suggestions")
(defface lsp-inline-completion-overlay-face '((t :inherit shadow)) "\
Face for the inline code suggestions overlay." :group 'lsp-mode)
